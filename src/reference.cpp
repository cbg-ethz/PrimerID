...
std::pair<int, int> record::hamming_distance(const std::string& read, const std::vector<int>& heterozygous_loci) const
{
	int mismatches = 0;
	int valid = 0;
...
//reference
int hamming_distance(const std::string& strRead, const std::string& strRef)
{
	int mismatches = 0;
	for (int i = 0; i < strRead.length(); ++i)
		mismatches += ((strRead[i] != 'N') && (strRef[i] != 'N') && (strRead[i] != strRef[i]));
...
void reference::display_strains_hetero() const
{
	...
	for (const record& i : all_reference_strains)
	{
		...
		if (i.frequency != 0)
			std::cout << '\t' << std::fixed << std::setprecision(1) << i.frequency*100;
...
void reference::assign_counts(const std::string& read, bool consensus)
{
	...
	std::pair<int, int> hamming;
	
	for (int i = 0; i < K; ++i)
	{
		hamming = all_reference_strains[i].hamming_distance(read, heterozygous_loci);
		
		// minimum number of valid bases to call a reference
		if (hamming.second < reference::min_valid)
			break;
		
		if (hamming.first < min)
		{
			min = hamming.first;
			index = i;
		}
